...
def compare_energy_generation(coordinates, tilt, azimuth, nominal_power, real_energy_last_year, month):
    ...
    if real_energy_last_year < estimated_generation_sum*0.9  or real_energy_last_year > estimated_generation_sum*1.1:
        ...
        suggestions = "The system is performing well compared with expected data."
    result = find_highest_energy_day(estimated_generation)
    ...
    return suggestions, round(estimated_generation_sum, 1), generation_data, day_generation


def get_all_month_data(daysMaxGeneration, estimated_generation): 
    generation_data = []
    for month, data in daysMaxGeneration.items():
        highest_day_index = (estimated_generation.index.month == month) & (estimated_generation.index.day == data['day'])
...
def get_month_data(daysMaxGeneration, estimated_generation, month):
    ...
    generation_data = []
    highest_day_index = (estimated_generation.index.month == month) & (estimated_generation.index.day == daysMaxGeneration[month]['day'])
    highest_day_profile = estimated_generation[highest_day_index]
    series = []
    for i in range(len(highest_day_profile.index.hour)):
        series.append({ "name" : int(highest_day_profile.index.hour[i]), "value" : float(highest_day_profile.values[i]) })
    generation_data.append({ "name": f"Month {month}, Day {daysMaxGeneration[month]['day']}", "series" : series })
    return generation_data, float(day_generation)
